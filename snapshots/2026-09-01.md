# TerminalFix Snapshot — 2026-09-01 UTC
...
- 페치 차단: theregister.com, gbhackers.com, cyberpress.org, dev.to, cybersecuritynews.com, develeap.com, huntress.com
- seed: 0 / 신규 검증: 0 / 미검증: 0

## 캠페인 개요
TerminalFix는 ClickFix 소셜엔지니어링 기법의 변종으로, 피해자를 Windows Run 대화상자 대신 Windows Terminal/PowerShell로 유도해 더 복잡한 다단계 스크립트를 실행시킨다. 손상된 정상 웹사이트에 가짜 Cloudflare Turnstile CAPTCHA를 띄워 PowerShell 명령을 클립보드에 심고, 이를 붙여넣도록 유도한다. 이후 DLL 사이드로딩, PNG 스테가노그래피, 광범위한 AD 정찰, 그리고 커스텀 역방향 터널 임플란트를 통한 지속적 네트워크 레벨 프록시 접근으로 이어지는 다단계 침투 체인이다. 전날(2026-08-31) 대비 변동 없음 — Microsoft 원문(2026-08-28)을 재확인했으며, 2차 보도(The Register, GBHackers, CyberPress, dev.to, CyberSecurityNews, Develeap, Huntress)는 모두 이번 실행에서도 페치가 차단되어 검색 스니펫 수준의 확인만 가능했다.
...
- 신규 인프라: 없음
- 신규 해시: 없음
- 승격(seed→검증): 없음 (모든 항목 전일 실행에서 이미 검증 완료)
- 폐기(sinkhole/takedown): 없음
- 신규 보고서: 없음 — Microsoft 원문 재확인 외 2차 보도 페치는 전부 차단됨

## 인접 캠페인 (참고)
- **DOUBLECUP** (2026-08, 검색 스니펫만 확인·본문 미페치) — 러시아계 Loader-as-a-Service. ClickFix 유인으로 브라우저 캐시에 스테가노그래피 PNG를 심고 CountLoader 및 신규 RAT `DeviceManager` 배포. TerminalFix와 무관한 별도 캠페인이나, PNG 스테가노그래피+ClickFix 기법 트렌드 참고용으로 기록. TerminalFix IOC 목록에는 포함하지 않음.
...
- [Attack hides malware in PNGs and drops custom reverse tunnel on victims' machines](https://www.theregister.com/security/2026/08/31/attack_hides_malware_in_pngs_and_drops_custom_reverse_tunnel_on_victims_machines/) — 2026-08-31 — blocked
...
- [TerminalFix: Fake CAPTCHA to PNG Steganography and WebSocket Reverse Tunnels](https://dev.to/anoymask/terminalfix-fake-captcha-to-png-steganography-and-websocket-reverse-tunnels-1p29) — 2026-08 — blocked
...
- [ClickFix Gets Creative: Malware Buried in Images](https://www.huntress.com/blog/clickfix-malware-buried-in-images) — 2026 — blocked (DOUBLECUP, 인접 캠페인)
