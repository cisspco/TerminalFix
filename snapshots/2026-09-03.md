# TerminalFix Snapshot — 2026-09-03 UTC
...
- 페치 차단: hothardware.com, redmondmag.com, scworld.com, isc.sans.edu, cyberpress.org, techradar.com, cybersecuritynews.com
- seed: 0 / 신규 검증: 0 / 미검증: 0

## 캠페인 개요
TerminalFix는 ClickFix 소셜엔지니어링 기법의 변종으로, 피해자를 Windows Run 대화상자 대신 Windows Terminal/PowerShell로 유도해 더 복잡한 다단계 스크립트를 실행시킨다. 손상된 정상 웹사이트에 가짜 Cloudflare Turnstile CAPTCHA를 띄워 PowerShell 명령을 클립보드에 심고, 이를 붙여넣도록 유도한다. 이후 DLL 사이드로딩, PNG 스테가노그래피, 광범위한 AD 정찰, 그리고 커스텀 역방향 터널 임플란트를 통한 지속적 네트워크 레벨 프록시 접근으로 이어지는 다단계 침투 체인이다. 전날(2026-09-02) 대비 IOC 변동 없음. Microsoft 원문(2026-08-28)을 재확인했으며, BYOVD/취약 드라이버(예: `ardrv.sys`, OPSWAT AppRemover) 및 권한 상승을 언급하는 일부 검색 스니펫(hothardware.com 등, 이번 실행에서 페치 차단됨)이 발견되었으나, Microsoft 원문을 이번 실행에서 직접 재확인한 결과 해당 내용은 포함되어 있지 않다 — 원 소스와 불일치하므로 IOC/공격 체인에 반영하지 않음 (⚠️ 미검증·미반영, 향후 실행에서 원문 페치로 재확인 필요).
...
- 신규 보고서: 없음 — Microsoft 원문만 페치 성공, 2차 보도(hothardware, redmondmag, scworld, isc.sans.edu, cyberpress, techradar, cybersecuritynews)는 모두 이번 실행에서 차단됨. 검색 스니펫에서 BYOVD(`ardrv.sys`, OPSWAT AppRemover)를 통한 권한 상승·보안 소프트웨어 비활성화 주장이 발견되었으나 Microsoft 원문 재확인 결과 불일치 — IOC에 미반영, 다음 실행에서 원문 소스로 재확인 필요

## 인접 캠페인 (참고)
- **DOUBLECUP** — 러시아계 Loader-as-a-Service. ClickFix 유인으로 브라우저 캐시에 스테가노그래피 PNG(BGRA 픽셀의 red 채널에 XOR 인코딩)를 심고 CountLoader 및 RAT `DeviceManager` 배포(활동 시작 2026년 6월경 추정). TerminalFix와 무관한 별도 캠페인이나, PNG 스테가노그래피+ClickFix 기법 트렌드 참고용으로 기록. TerminalFix IOC 목록에는 포함하지 않음.
...
- [TerminalFix Attacks Use Fake CAPTCHAs To Secretly Hack Windows PCs](https://hothardware.com/news/terminalfix-attacks-use-fake-captchas-to-secretly-hack-windows-pcs) — 2026-09 — blocked
- [Microsoft Warns TerminalFix Turns Fake CAPTCHA Checks Into Network Access](https://redmondmag.com/articles/2026/09/02/microsoft-warns-terminalfix-turns-fake-captcha-checks-into-network-access.aspx) — 2026-09-02 — blocked
- [Microsoft identifies 'TerminalFix' campaign spreading Python reverse tunnel](https://www.scworld.com/news/microsoft-identifies-terminalfix-campaign-spreading-python-reverse-tunnel) — 2026-09 — blocked
- [SANS Stormcast Tuesday, September 1st, 2026: TerminalFix Malware](https://isc.sans.edu/podcastdetail/10076) — 2026-09-01 — blocked
...
- [Fake Cloudflare CAPTCHA page laden with malware uncovered in the wild](https://www.techradar.com/pro/security/fake-cloudflare-captcha-page-laden-with-malware-uncovered-in-the-wild-heres-how-to-stay-secure-and-safe) — 2026-08/09 — blocked
- [Hackers Use Fake Cloudflare CAPTCHA to Deploy Reverse Tunnel Into Corporate Networks](https://cybersecuritynews.com/fake-cloudflare-captcha/) — 2026-08/09 — blocked
