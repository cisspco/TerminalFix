# TerminalFix Snapshot — 2026-09-05 UTC
...
- 페치 차단: malwarebytes.com, theregister.com, scworld.com, cyberpress.org, redmondmag.com, gbhackers.com, socprime.com
- seed: 0 / 신규 검증: 0 / 미검증: 0

## 캠페인 개요
TerminalFix는 ClickFix 소셜엔지니어링 기법의 변종으로, 피해자를 Windows Run 대화상자 대신 Windows Terminal/PowerShell로 유도해 더 복잡한 다단계 스크립트를 실행시킨다. 손상된 정상 웹사이트에 가짜 Cloudflare Turnstile CAPTCHA를 띄워 PowerShell 명령을 클립보드에 심고, 이를 붙여넣도록 유도한다. 이후 DLL 사이드로딩, PNG 스테가노그래피, 광범위한 AD 정찰, 그리고 커스텀 역방향 터널 임플란트를 통한 지속적 네트워크 레벨 프록시 접근으로 이어지는 다단계 침투 체인이다. 전날(2026-09-04) 대비 IOC 변동 없음 — Microsoft 원문(2026-08-28)을 재확인했으며 baseline과 완전히 일치함. 검색 스니펫상으로는 Malwarebytes(2026-09), SC Media/Redmondmag(2026-09-02) 등 2차 보도가 다수 확인되었으나 새로운 인프라·해시·IOC는 보고되지 않았고, 이번 실행에서는 모두 페치 차단됨.
...
- 신규 보고서: 없음 — Microsoft 원문만 페치 성공(내용은 baseline과 동일), 2차 보도(malwarebytes.com, theregister.com, scworld.com, cyberpress.org, redmondmag.com, gbhackers.com, socprime.com)는 검색으로 다수 확인되었으나 새로운 IOC 없이 전부 이번 실행에서 egress 프록시에 의해 차단됨
